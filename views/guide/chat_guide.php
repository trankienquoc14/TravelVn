...
<style>
    ...
    .chat-input-wrapper button.tool-btn {
        background: transparent; border: none; color: #64748b; padding: 6px 10px; border-radius: 50%; transition: 0.2s;
    }
    .chat-input-wrapper button.tool-btn:hover { color: var(--guide-primary); background: #e2e8f0; }
    ...
    .session-item.active-session { background: #f0f9ff; border-left: 4px solid var(--guide-primary) !important; }
    
    /* CSS Xử lý Responsive Mobile */
    @media (max-width: 767.98px) {
        .chat-card { height: 85vh !important; }
        .chat-card .col-md-4 { height: 35% !important; border-right: none !important; border-bottom: 2px solid var(--guide-border) !important;}
        .chat-card .col-md-8 { height: 65% !important; }
    }

    /* 🔥 CSS NÂNG CẤP MEDIA */
    .chat-image { max-width: 220px; border-radius: 12px; cursor: pointer; border: 2px solid var(--guide-border); }
    ...
    .guide-msg-me .file-box a { color: white; text-decoration: underline; }
    .guide-msg-customer .file-box a { color: var(--guide-primary); text-decoration: none; }
    .msg-location { color: inherit; text-decoration: underline; font-weight: bold; }
    
    #guideFilePreview { font-size: 0.85rem; padding: 6px 12px; background: #e0f2fe; border: 1px dashed #0ea5e9; border-radius: 8px; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center; }
    ...
    <div class="chat-card">
        <div class="row g-0 h-100">
            ...
            <div class="col-md-8 d-flex flex-column h-100" style="background: #f8fafc;">
                ...
                <div id="guideChatBody" class="p-4 flex-grow-1 overflow-auto d-flex flex-column gap-3" style="min-height: 0;">
                ...
                <div id="chatFooter" class="p-3 bg-white border-top d-none">
                    <form id="guideChatForm">
                        ...
                        <div class="chat-input-wrapper d-flex align-items-center ps-2 pe-1 py-1" style="background: #f1f5f9;">
                            <button type="button" class="tool-btn" title="Gửi tệp" onclick="document.getElementById('guideChatFile').click()"><i class="bi bi-paperclip fs-5"></i></button>
                            <button type="button" class="tool-btn" title="Chia sẻ vị trí" onclick="sendGuideLocation()"><i class="bi bi-geo-alt fs-5"></i></button>
                            <button type="button" class="tool-btn" id="guideRecordBtn" title="Ghi âm"><i class="bi bi-mic fs-5" id="guideRecordIcon"></i></button>

                            <input type="text" id="guideChatInput" class="form-control border-0 shadow-none bg-transparent px-2" 
...
<script>
        ...
        fetch('guide.php?action=getSessions&t=' + new Date().getTime())
            ...
            .then(data => {
                ...
                data.forEach(s => {
                    ...
                    // Xem trước dạng tin nhắn (File/Voice/Map)
                    let previewText = s.message;
                    if (!previewText) previewText = "📎 Đã gửi tệp đính kèm";
                    if (previewText.includes('https://maps.google.com/?q=$')) previewText = "📍 Đã chia sẻ vị trí";
    ...
    async function openChat(sessionId, senderName) {
        ...
        clearGuideFilePreview(); // Xóa preview cũ
    ...
    // 3. VẼ TIN NHẮN THEO ĐỊNH DẠNG (FILE, AUDIO, MAP)
    function appendMessageUI(type, content, messageType = 'text') {
        ...
        // 🔥 BẮT LỖI QUAN TRỌNG: Xử lý dữ liệu rỗng từ backend
    ...
    // 4. XỬ LÝ PREVIEW & UPLOAD TỆP ĐÍNH KÈM
        ...
        if (guideSelectedFile) {
            ...
            fetch('guide.php?action=uploadFile', { method: 'POST', body: formData })
                ...
                .then(data => { 
                    ...
                    if(data && data.url) { 
                        // 🔥 Tự động dự đoán đuôi file nếu backend trả thiếu data.type
   ...
   // 7. GHI ÂM (VOICE)
    ...
    // 8. PUSHER REALTIME
    var guidePusher = new Pusher('e5405b1b2139fed6f8bc', { cluster: 'ap1', forceTLS: true });
