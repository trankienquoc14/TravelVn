...
<style>
    /* --- PREMIUM FOOTER --- */
    .footer-custom {background-color: #ceefff; border-top: 1px solid #e2e8f0; color: #475569; font-family: 'Inter', 'Segoe UI', sans-serif; margin-top: 60px;}
    .footer-top {padding: 60px 0 40px;}
    .footer-brand {font-size: 1.8rem; font-weight: 800; color: #0194f3; margin-bottom: 15px; display: flex; align-items: center; gap: 8px;}
    .footer-title {font-weight: 700; color: #1a202c; margin-bottom: 20px; font-size: 1.15rem;}
    .footer-links, .footer-contact {list-style: none; padding: 0; margin: 0;}
    .footer-links li, .footer-contact li {margin-bottom: 15px;}
    .footer-links a {color: #64748b; text-decoration: none; transition: all 0.3s ease; display: inline-block;}
    .footer-links a:hover {color: #0194f3; transform: translateX(5px);}
    .footer-contact li {display: flex; align-items: flex-start; gap: 12px; line-height: 1.6;}
    .footer-contact i {color: #0194f3; font-size: 1.2rem; margin-top: 2px;}
    
    /* Social Icons */
    .social-links {display: flex; gap: 12px; margin-top: 25px;}
    .social-icon {display: inline-flex; align-items: center; justify-content: center; width: 40px; height: 40px; border-radius: 50%; background-color: #e2e8f0; color: #475569; text-decoration: none; transition: 0.3s; font-size: 1.1rem;}
    .social-icon:hover {background-color: #0194f3; color: white; transform: translateY(-4px); box-shadow: 0 4px 10px rgba(1, 148, 243, 0.3);}
    
    /* Footer Bottom */
    .footer-bottom {background-color: #e2f8ff; padding: 20px 0; font-size: 0.95rem; border-top: 1px solid #e2e8f0;}
    .payment-methods {display: flex; gap: 15px; align-items: center;}
    .payment-methods i {font-size: 1.8rem; color: #94a3b8; transition: 0.3s;}
    ...
    .chat-widget {position: fixed; bottom: 25px; right: 25px; z-index: 1050; font-family: 'Inter', sans-serif;}
    .chat-button {width: 60px; height: 60px; border-radius: 50%; background: linear-gradient(135deg, #0194f3, #00d2ff); color: white; border: none; box-shadow: 0 4px 20px rgba(1, 148, 243, 0.4); font-size: 26px; cursor: pointer; transition: all 0.3s ease; display: flex; align-items: center; justify-content: center;}
    .chat-button:hover {transform: scale(1.08) rotate(5deg); box-shadow: 0 6px 24px rgba(1, 148, 243, 0.5);}
    
    .chat-panel {display: none; width: 360px; height: 520px; background: white; border-radius: 20px; box-shadow: 0 12px 40px rgba(0, 0, 0, 0.16); flex-direction: column; position: absolute; bottom: 75px; right: 0; overflow: hidden; border: 1px solid rgba(226, 232, 240, 0.8); transition: all 0.3s ease;}
    .chat-header {background: linear-gradient(135deg, #0194f3, #00b4f3); color: white; padding: 16px 20px; font-weight: 600; display: flex; justify-content: space-between; align-items: center;}
    
    .chat-body {flex: 1; padding: 20px; overflow-y: auto; background: #f8fafc; display: flex; flex-direction: column; gap: 14px; scroll-behavior: smooth;}
    .chat-body::-webkit-scrollbar {width: 5px;}
    .chat-body::-webkit-scrollbar-track {background: #f1f5f9;}
    .chat-body::-webkit-scrollbar-thumb {background: #cbd5e1; border-radius: 4px;}

    .chat-footer {padding: 14px; border-top: 1px solid #f1f5f9; background: white;}
    .chat-footer form {display: flex; flex-direction: column; gap: 8px; margin: 0;}
    ...
    .chat-input-wrapper {
        ...
        gap: 6px; 
        background: #f1f5f9; 
        padding: 4px 6px; /* Chỉnh lại padding để nhường chỗ cho nút */
        border-radius: 24px; 
    ...
    .chat-input {
        flex: 1; 
        min-width: 0; /* QUAN TRỌNG: Ép ô input tự thu nhỏ lại để không đẩy nút gửi ra ngoài */
        border: none; 
    ...
    .chat-footer button {
        flex-shrink: 0; /* QUAN TRỌNG: Không cho phép các nút bấm bị bóp méo hoặc ép nhỏ */
        width: 36px; 
        height: 36px; 
        border: none; 
        border-radius: 50%; 
    ...
    .chat-footer .chat-submit {
        ...
        width: 38px; 
        height: 38px;
        margin-left: 2px; /* Tạo khoảng cách nhẹ với ô input */
    }
    
    .msg-bubble {max-width: 80%; padding: 12px 16px; border-radius: 18px; font-size: 0.95rem; line-height: 1.45; word-wrap: break-word; position: relative;}
    .msg-customer {background: linear-gradient(135deg, #0194f3, #00b4f3); color: white; align-self: flex-end; border-bottom-right-radius: 4px;}
    .msg-admin {background: white; color: #1e293b; align-self: flex-start; border-bottom-left-radius: 4px; border: 1px solid #e2e8f0;}
    
    .chat-image {max-width: 100%; border-radius: 12px; cursor: pointer;}
    .chat-audio {width: 100%; max-width: 220px; margin-top: 4px;}
    .file-box {padding: 10px 14px; border-radius: 12px; background: rgba(0, 0, 0, 0.04); display: flex; align-items: center; gap: 8px;}

    #filePreview {padding: 8px 12px; background: #f0fdf4; border: 1px dashed #4ade80; border-radius: 10px; display: flex; justify-content: space-between; align-items: center; font-size: 0.85rem;}
    #filePreview .remove-preview {cursor: pointer; color: #ef4444; font-weight: bold;}

    .recording {background: #ef4444 !important; color: white !important; animation: pulse 1.2s infinite;}
    @keyframes pulse {
        0% {transform: scale(1);}
    ...
    .chat-loading {display: inline-flex; align-items: center; gap: 4px; padding: 8px 12px;}
    .chat-loading span {width: 6px; height: 6px; background: #94a3b8; border-radius: 50%; animation: blink 1.4s infinite both;}
                ...
                <div class="col-lg-3 col-md-6">
                    ...
                    <ul class="footer-contact">
                        <li>
                            ...
                            <a href="https://www.google.com/maps/search/?api=1&query=123+Nguyễn+Văn+Bảo,+Quận+Gò+Vấp,+TP.HCM" target="_blank" rel="noopener noreferrer" style="text-decoration: none; color: inherit;">
...
?>
    <div class="chat-widget" id="chatWidgetContainer">
        <div class="chat-panel" id="chatPanel">
            ...
            <div class="chat-body" id="chatBody">
                <div class="text-center text-muted small mt-2 mb-3">Chào mừng bạn đến với TravelVN. Chúng tôi có thể giúp gì cho bạn?</div>
            </div>
            
            <div class="chat-footer">
                <form id="chatForm" autocomplete="off">
                    <input type="hidden" id="chatDepartureId" value="<?= htmlspecialchars($booking['departure_id'] ?? '') ?>">
        ...
        <button class="chat-button position-relative" onclick="toggleChat()" id="chatMainButton">
            ...
            <span id="bubble-chat-badge" class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger d-none" style="font-size: 0.75rem; border: 2px solid white; padding: 4px 6px;">0</span>
        </button>
    </div>
    ...
    function appendMessage(sender, content, messageType = 'text') {
        ...
        switch (messageType) {
            ...
            case 'audio':
                ...
                // 🔥 SỬA: Xóa type="audio/webm" đi để hỗ trợ mọi trình duyệt (iOS/Android/PC)
    ...
    function loadChatHistory() {
        ...
        .then(data => {
            ...
            if (data.length > 0) { mySessionId = data[0].session_id; }
            data.forEach(msg => {
                let content = msg.message_type === 'location' ? `https://maps.google.com/?q=${msg.latitude},${msg.longitude}` : (msg.message || msg.file_url);
    ...
    chatForm.addEventListener('submit', function(e) {
        ...
        // Trạng thái 1: Có chọn file/ảnh đính kèm
        if (selectedFileObject) {
            showLoadingIndicator(); // Hiện bong bóng loading
            const formData = new FormData();
            formData.append('file', selectedFileObject);
            formData.append('departure_id', departureId);
            clearFilePreview(); // Xóa khung preview ở giao diện

            fetch('index.php?action=uploadFile', { method: 'POST', body: formData })
            .then(res => res.json())
            .then(data => { 
                console.log(data);
                // 🔥 BƯỚC QUAN TRỌNG: LUÔN XÓA LOADING KHI SERVER TRẢ VỀ KẾT QUẢ
                const loader = document.getElementById('temp-chat-loader');
                if(loader) loader.remove();

                // Kiểm tra xem backend trả về thành công (có url) hay báo lỗi
                if (data && data.url) { 
            ...
            .catch(err => {
                // 🔥 BƯỚC QUAN TRỌNG: NẾU MẠNG LỖI CŨNG PHẢI XÓA LOADING
                const loader = document.getElementById('temp-chat-loader');
                if(loader) loader.remove();
                alert("Lỗi kết nối máy chủ khi tải lên file!");
            });
        } 
        // Trạng thái 2: Chỉ gửi tin nhắn văn bản thông thường
        else if (textMsg) {
            ...
            .then(res => res.json())
            .then(data => {
                // 1. Cập nhật mySessionId ngay lập tức nếu backend gửi về
                if (data && data.session_id) {
                    mySessionId = data.session_id;
                }
                // 2. Nếu là tin nhắn đầu tiên, tự in câu Bot ra giao diện (vì mảng Pusher có thể bị miss)
                if (data && data.is_first) {
                    appendMessage('admin', "🤖 Xin chào! TravelVN đã nhận được tin nhắn của bạn. Chúng tôi sẽ phản hồi sớm nhất ❤️", 'text');
                }
            });
        }
    }); 
...
if(recordBtn) {
    recordBtn.onclick = async () => {
        ...
        try {
           ...
           mediaRecorder.onstop = () => {
                ...
                showLoadingIndicator(); // Hiện loading 3 dấu chấm

                // 🔥 SỬA: Lấy định dạng chuẩn của trình duyệt đang dùng (tránh lỗi trên iOS/Safari)
                const audioType = mediaRecorder.mimeType || 'audio/webm';
                const audioBlob = new Blob(audioChunks, { type: audioType });
                
                const formData = new FormData();
                // Đặt tên file chung chung, backend/Cloudinary sẽ tự xử lý phần mở rộng
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const pusher = new Pusher('e5405b1b2139fed6f8bc', { cluster: 'ap1', forceTLS: true });
        const channel = pusher.subscribe('live-chat');
        channel.bind('new-message', function(data) {
            if (data && data.session_id === mySessionId && data.sender_type !== 'customer') {
                if (isChatOpen) {
                    appendMessage('admin', data.message || data.file_url, data.message_type);
                    fetch('index.php?action=markAsRead', { method: 'POST' });
                } else { updateCustomerChatBadge(); }
            }
        });
    });
    </script>
